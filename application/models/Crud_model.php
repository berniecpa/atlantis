...
return $this->db->get('listing');
  ...
  function update_listing($listing_id = "")
  {
    ...
    if (is_array($this->input->post('amenities')) && sizeof(sanitizer($this->input->post('categories'))) > 0) {
      $data['categories'] = $this->make_json(sanitizer($this->input->post('categories')));
    } else {
      $data['categories'] = json_encode(array());
    }
  ...
  public function update_frontend_settings()
  {
    ...
    $abouts = $this->input->post('about_us', false);
    $abouts = str_replace('<a href="', "", $abouts);
    $abouts = str_replace('target="_blank">', "", $abouts);
    $data['description'] = $abouts;
    $this->db->where('type', 'about_us');
    $this->db->update('frontend_settings', $data);

    $terms_and_condition = $this->input->post('terms_and_condition', false);
    $terms_and_condition = str_replace('<a href="', "", $terms_and_condition);
    $terms_and_condition = str_replace('target="_blank">', "", $terms_and_condition);
    $data['description'] = $terms_and_condition;
    $this->db->where('type', 'terms_and_condition');
    $this->db->update('frontend_settings', $data);


    $privacy_policy = $this->input->post('privacy_policy', false);
    $privacy_policy = str_replace('<a href="', "", $privacy_policy);
    $privacy_policy = str_replace('target="_blank">', "", $privacy_policy);
    $data['description'] = $privacy_policy;
    $this->db->where('type', 'privacy_policy');
    $this->db->update('frontend_settings', $data);

    $faq = $this->input->post('faq', false);
    $faq = str_replace('<a href="', "", $faq);
    $faq = str_replace('target="_blank">', "", $faq);
    ...
    $cookie_policy = $this->input->post('cookie_policy', false);
    $cookie_policy = str_replace('<a href="', "", $cookie_policy);
    $cookie_policy = str_replace('target="_blank">', "", $cookie_policy);
