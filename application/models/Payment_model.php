...
class Payment_model extends CI_Model {
    ...
    public function stripe_payment($session_id = "", $paid_amount = "") {

        $stripe_keys = json_decode(get_settings('stripe'), true);

        if($stripe_keys[0]['testmode'] == 'on'){
          $public_key = $stripe_keys[0]['public_key'];
          $secret_key = $stripe_keys[0]['secret_key'];
        }else{
